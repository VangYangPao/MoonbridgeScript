#!/bin/bash
# Moonbridge Masternode Setup Script V1.4 for Ubuntu 16.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'moonbridged' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop Moonbridged${NC}"
        moonbridge-cli stop
        sleep 30
        if pgrep -x 'moonbridge' > /dev/null; then
            echo -e "${RED}Moonbridged daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            pkill moonbridged
            sleep 30
            if pgrep -x 'moonbridged' > /dev/null; then
                echo -e "${RED}Can't stop moonbridged! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${YELLOW}Moonbridge Masternode Setup Script V1.5 for Ubuntu 16.04 LTS${NC}"
...
rm -rf moonbridge-ubuntu.tar.gz
 ...
 sudo cp ~/MoonbridgeMasternodeSetup/moonbridge/moonbridge* /usr/bin/
 sudo chmod 755 -R ~/MoonbridgeMasternodeSetup
 sudo chmod 755 /usr/bin/moonbridge* 
 
 # Deploy masternode monitoring script
 cp ~/MoonbridgeMasternodeSetup/moonbridgemon.sh /usr/local/bin
 sudo chmod 711 /usr/local/bin/moonbridgemon.sh
 
 #Create datadir
 if [ ! -f ~/.moonbridge/moonbridge.conf ]; then 
 	sudo mkdir ~/.moonbridge
 fi
clear
echo -e "${YELLOW}Creating moonbridge.conf...${NC}"
...
EOF
    ...
    sudo chmod 755 -R ~/.moonbridge/moonbridge.conf
...
echo -ne '\n'
    ...
    genkey=$(moonbridge-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create moonbridge.conf
    stop_daemon
    sleep .5
fi
# Create moonbridge.conf
cat <<EOF > ~/.moonbridge/moonbridge.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
...
#Finally, starting daemon with new moonbridge.conf
moonbridged --daemon
sleep 5

#Setting auto start cron job for moonbridged
cronjob="@reboot sleep 30 && moonbridged"
crontab -l > tempcron
...
To introduce your new masternode to the Moonbridge network, you need to
...
Currently your masternode is syncing with the Moonbridge network...
...
========================================================================
To view masternode configuration produced by this script in moonbridge.conf:
${YELLOW}cat ~/.moonbridge/moonbridge.conf${NC}
Here is your moonbridge.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.moonbridge/moonbridge.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit moonbridge.conf, first stop the moonbridged daemon,
then edit the moonbridge.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the moonbridged daemon back up:
             to stop:   ${YELLOW}moonbridge-cli stop${NC}
             to edit:   ${YELLOW}nano ~/.moonbridge/moonbridge.conf${NC}
             to start:  ${YELLOW}moonbridged${NC}
========================================================================
To view moonbridge debug log showing all MN network activity in realtime:
             ${YELLOW}tail -f ~/.moonbridge/debug.log${NC}
========================================================================
...
sync status etc. in real-time, run the nodemon.sh script:
                 ${YELLOW}moonbridgemon.sh${NC}
or just type 'node' and hit <TAB> to auto-complete script name.
========================================================================
Enjoy your Moonbridge Masternode and thanks for using this setup script!
"
sleep 30
# Run moonbridgemon.sh
moonbridgemon.sh
